...
interface TopBarProps {
  ...
  llm: {
    ...
    openLoadingDialog: (
      type?: 'insert' | 'analysis' | 'summary' | 'improve' | 'translate'
    ) => void;
...
export default function TopBar({ title, aiHistory, llm }: TopBarProps) {
  const handleLLMResponse = (result: LLMResponse, usedPrompt?: string) => {
    ...
    if (outcome.status === 'success') {
      llm.setDialogResult(
        data?.rewritten_text || 'Nessun testo generato.',
        usedPrompt
      );
  ...
  return (
    <>
        ...
        <DialogContent
          ...
          {isEditingAI && (
            <Alert severity='warning'>
              Attenzione: stai rigenerando un testo creato dall'IA. Le tue
              modifiche manuali su questa sezione andranno perse.
            </Alert>
          )}
          <TextField
            autoFocus
      ...
      {/* --- DIALOG MIGLIORA --- */}
      <Dialog open={openCriterion} onClose={() => setOpenCriterion(false)} fullWidth maxWidth='sm'>
